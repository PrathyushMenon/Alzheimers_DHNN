...
class MultiModalDataset(Dataset):
    def __init__(self, csv_file, transform=None, dataset='train', modality='t1'):
        ...
        self.data = []
        if {"split", "label", "t1", "taupet", "fod"}.issubset(df.columns):
            df = df[df["split"].astype(str).str.lower() == dataset.lower()]
            for _, row in df.iterrows():
                self.data.append(
                    {
                        "label": int(row["label"]),
                        "t1": str(row["t1"]),
                        "taupet": str(row["taupet"]),
                        "fod": str(row["fod"]),
                    }
                )
        else:
        ...
        if {"split", "label", "t1", "taupet", "fod"}.issubset(pd.read_csv(csv_file, nrows=1).columns):
            train_data = self.data if dataset == 'train' else []
            validation_data = self.data if dataset == 'val' else []
            test_data = self.data if dataset == 'test' else []
        else:
...
def get_loader(args):
    ...
    label_path = getattr(args, "json_list", "") or os.environ.get("SWIN_FOD_CSV", "")
